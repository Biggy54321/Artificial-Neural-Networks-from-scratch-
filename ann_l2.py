...
class ANN:
    ...
    def forward_prop_mem(self, x):
        ...
        # feed the input x through all the layers one by one
        for i in range(self.no_layers):
            wt_sum[i + 1] = act[i] @ self.weights[i] + self.bias[i]
    ...
    def back_prop(self, delta, wt_sum, act, reg_term, batch_size):
        ...
        while i > 0:
            ...
            dw[i - 1] = (act[i - 1].T @ delta) / batch_size + (reg_term / batch_size) * self.weights[i - 1]
            # update delta
    ...
    def train(self, x, y, learn_rate, epochs, batch_size, reg_term):
        ...
        d. reg_term - The regularization constant to be multiplied to the norm of weights in the cost
        ...
        for e in range(epochs):
            ...
            for b in range(0, len(x), batch_size):
                ...
                wt_sum, act = self.forward_prop_mem(x[b : b + batch_size])
